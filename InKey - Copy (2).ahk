...
ProcessTinkerFile(kk) {  ; Generate an AHK file from the TINKER file
	...
	if (fpos) {
		...
		; Get CapsChars setting
		if (RegExMatch(settings, "Om)^\s*CapsChars:\s+(\S*)$", match)) {
			capsChars := match.Value(1)
		}
	...
	for each key in capChars {
		if (RegExMatch(tinkerLines, rulesRE(key), matchLo)
				or RegExMatch(tinkerLines, rulesRE("+" key), matchUp)) {
			tinkerLines := SubStr(tinkerlines, 1, matchLo.Pos(0)-1) . SubStr(tinkerLines, matchLo.Pos(0)+matchLo.Len(0)) . SubStr(tinkerlines, 1, matchUp.Pos(0)-1) . SubStr(tinkerLines, matchUp.Pos(0)+matchUp.Len(0))
			loTranslate := translateRules(matchLo.Value(2))
			upTranslate := translateRules(matchUp.Value(2))
			FileAppend("$" key "::`n$+" key "::if (UseUpperCase()) {`n`t" upTranslate "`n} else {`n`t" loTranslate "`n}`n"
		} else {
			Warn "CapChars included '" key "', but this requires handlers for both " key " and +" key
		}
	}
function rulesRE(key) {
	return 	"O)^(?:\r?\n?)*(\Q" key "\E)\s*>\s*(\S.*(\r?\n\s+.*)*\r?\n?)"
}
	...
	while (StrLen(tinkerLines) > 1) {
		...
		if (RegExMatch(tinkerLines, "O)^(?:\r?\n?)*(\S+)\s*>\s*(\S.*(\r?\n\s+.*)*\r?\n?)", match)) {
			...
			for index, element in rules {
				if (index > 1)
					FileAppend %A_Space% ||%A_Space%, %AHKFile%
				...
				; if (RegExMatch(element, "Oi)^(map|loopMap|multiTapMap)\s*((【.*?】\s*?)+)\s*$", m2)) {
					...
					FileAppend InCase(%mapType%(%mapParams%))`n, %AHKFile%

				; multiTapSend 【 】
				; } else if (RegExMatch(element, "Oi)^multiTapSend\s*【(.*?)】\s*$", m2)) {
					; local rep := m2.Value(1)
					; FileAppend InCase(MultiTapSend("%rep%"))`n, %AHKFile%
				...
				; (After 【 】)? Replace 【 】 with 【 】 (usingMap|usingLoopMap 【 】+)?
					...
					FileAppend InCase(%aft%Replace("%rep%") with("%wth%")%using%)`n, %AHKFile%

				; after 【 】 send 【 】
				} else if (RegExMatch(element, "Oi)^after\s*【(.*?)】\s*send\s*【(.*?)】\s*$", m2)) {
					local rep := m2.Value(1)
					local wth := m2.Value(2)
					FileAppend InCase(After("%rep%") thenSend("%wth%"))`n, %AHKFile%
