...
SendToInKey(msg, hwndIK, wParam=0, lParam=0)
{
	r := DllCall("PostMessage", UInt, hwndIK, UInt, msg, UInt, wParam, UInt, lParam)
...
RefreshList() {
	...
	LV_Delete()
	SetTitleMatchMode 3
	WinGet, AHKWindows, List, inkey.ahk
	;~ WinGet, AHKWindows, List, ahk_class AutoHotkey

	Loop %AHKWindows% {


	   r := SendToInkey(0xa000 + A_Index, AHKWindows%A_Index%)
	   OutputDebug % A_Index . "=" . AHKWindows%A_Index%   . "   r=" . r
		;Get process ID
		WinGet, AHKWindows%A_Index%_PID, PID, % "ahk_id " AHKWindows%A_Index%
		GetProcessMemoryInfo(AHKWindows%A_Index%_PID)

		;Get memory info
		LV_Add(0, AHKWindows%A_Index%_PID, GetScriptPathFromHwnd(AHKWindows%A_Index%), AHKWindows%A_Index%
