...
$0::InCase(After("[\x{2E5}-\x{2E9}]") thenSend(Char(0x2E9)))		       ; Continue a left stem contour
		||  InCase(After("[\x{A712}-\x{A716}]") thenSend(Char(0xA712)))		; Continue a right stem contour
		||  InCase(Map("# ˩", "& ꜒", "̊ ̏"))			            ;    #   2E9,   &   A712, 30A   30F
		||  Send("0")

$1::InCase(Replace("##") with(Char(0xF1F1)))  				                    ; Begin a level pitch contour after ##
		|| InCase(After("[\x{F1F1}-\x{F1F9}]") thenSend(Char(0xF1F1)))		; Continue a level pitch contour
		||  InCase(After("[\x{2E5}-\x{2E9}]") thenSend(Char(0x2E8)))		; Continue a left stem contour
		||  InCase(After("[\x{A712}-\x{A716}]") thenSend(Char(0xA713)))		; Continue a right stem contour
		||  InCase(Map("# ˨", "& ꜓", "̊ ̀", "̌ ᷈", "̄ ᷆", "́ ̂"))    ;  # 2E8, & A713, 030A 0300, 030C 1DC8, 0304 1DC6, 0301 0302
		||  Send("1")

$2::InCase(Replace("##") with(Char(0xF1F2)))                                 ; Begin a level pitch contour after ##
		|| InCase(After("[\x{F1F1}-\x{F1F9}]") thenSend(Char(0xF1F2)))		; Continue a level pitch contour
		||  InCase(After("[\x{2E5}-\x{2E9}]") thenSend(Char(0x2E7)))		; Continue a left stem contour
		||  InCase(After("[\x{A712}-\x{A716}]") thenSend(Char(0xA714)))		; Continue a right stem contour
		||  InCase(Map("# ˧", "& ꜔", "̊ ̄", "̀ ᷅", "́ ᷇"))   ; # 02E7,  & A714,  030A 0304,  0300 1DC5,  0301 1DC7
		||  Send("2")

$3::InCase(Replace("##") with(Char(0xF1F3)))                                    ; Begin a level pitch contour after ##
		|| InCase(After("[\x{F1F1}-\x{F1F9}]") thenSend(Char(0xF1F3)))		; Continue a level pitch contour
		|| InCase(After("[\x{2E5}-\x{2E9}]") thenSend(Char(0x2E6)))		; Continue a left stem contour
		|| InCase(After("[\x{A712}-\x{A716}]") thenSend(Char(0xA715)))		; Continue a right stem contour
		|| InCase(Map("# ˦", "& ꜕", "̊ ́", "̀ ̌", "̄ ᷄", "̂ ᷉"))  ;  # 02E6, & A715, 030A 0301, 0300 030C, 0304 1DC4, 0302 1DC9
		|| Send("3")

$4::InCase(Replace("##") with(Char(0xF1F4)))                                  ; Begin a level pitch contour after ##
		|| InCase(After("[\x{F1F1}-\x{F1F9}]") thenSend(Char(0xF1F4)))		; Continue a level pitch contour
		||  InCase(After("[\x{2E5}-\x{2E9}]") thenSend(Char(0x2E5)))		; Continue a left stem contour
		||  InCase(After("[\x{A712}-\x{A716}]") thenSend(Char(0xA716)))		; Continue a right stem contour
		||  InCase(Map("# ˥", "& ꜖", "̊ ̋"))		                     ;  # 02E5,, & A716, 030A 030B
		|| Send("4")

$5::InCase(Replace("##") with(Char(0xF1F5)))                                  ; Begin a level pitch contour after ##
		|| InCase(After("[\x{F1F1}-\x{F1F9}]") thenSend(Char(0xF1F5)))		; Continue a level pitch contour
		|| Send("5")

$6::InCase(Replace("##") with(Char(0xF1F6)))                                  ; Begin a level pitch contour after ##
		|| InCase(After("[\x{F1F1}-\x{F1F9}]") thenSend(Char(0xF1F6)))		; Continue a level pitch contour
		|| Send("6")

$7::InCase(Replace("##") with(Char(0xF1F7)))                                  ; Begin a level pitch contour after ##
		|| InCase(After("[\x{F1F1}-\x{F1F9}]") thenSend(Char(0xF1F7)))		; Continue a level pitch contour
		|| Send("7")

$8::InCase(Replace("##") with(Char(0xF1F8)))                                  ; Begin a level pitch contour after ##
		|| InCase(After("[\x{F1F1}-\x{F1F9}]") thenSend(Char(0xF1F8)))		; Continue a level pitch contour
		|| Send("8")

$9::InCase(Replace("##") with(Char(0xF1F9)))                                  ; Begin a level pitch contour after ##
		|| InCase(After("[\x{F1F1}-\x{F1F9}]") thenSend(Char(0xF1F9)))		; Continue a level pitch contour
		|| Send("9")
...
${::InCase(Map("\x{32A} \x{33A} \x{33B} \x{33C} \x{323}", "\ {"))		; combining_bridge_below |  combining_inverted_bridge_below |  combining_square_below |  combining_seagull_below |  combining_dot_below
	|| InCase(After("\p{L}\p{M}*") thenSend(Char(0x32A)))
	|| Send("{")
...
$*::InCase(Map("\ *", "\x{308} \x{33d} \x{306} \x{307} \x{310}"))		; combining_diaeresis |  combining_x_above |  combining_breve |  combining_dot_above |  combining_candrabindu
		|| InCase(After("\p{L}\p{M}*") thenSend(Char(0x308)) elseSend("*"))
...
$+::InCase(Map("\ +", "\x{31F} \x{31D} \x{318} \x{339}"))		;  ◌̟ ◌̝ ◌̘ ◌̹		combining_plus_sign_below |  combining_up_tack_below |  combining_left_tack_below |  combining_right_half_ring_below
		|| InCase(After("\p{L}\p{M}*") thenSend(Char(0x31F)) elseSend("+"))
...
$_::InCase(Map("\ _", "\x{320} \x{31E} \x{319} \x{31C} \x{327}"))		;|	◌̠ ◌̞ ◌̙ ◌̜ ◌̧		 combining_minus_sign_below |  combining_down_tack_below |  combining_right_tack_below |  combining_left_half_ring_below |  combining_cedilla
		|| InCase(After("\p{L}\p{M}*") thenSend(Char(0x320)) elseSend("_"))
...
$%::InCase(Map("b̤ ᶀ", "d̤ ᶁ", "f̤ ᶂ", "g̤ ᶃ", "k̤ ᶄ", "l̤ ᶅ", "m̤ ᶆ", "n̤ ᶇ", "p̤ ᶈ", "r̤ ᶉ", "s̤ ᶊ", "ʃ̤ ᶋ", "v̤ ᶌ", "x̤ ᶍ", "z̤ ᶎ"))  ; dieresis to palatal hooks	(This is a non-looping rota.)
		|| InCase(Map("\ %", "\x{325} \x{32C} \x{324}"))		; ◌̥ ◌̬ ◌̤		 combining_ring_below |  combining_caron_below |  combining_diaeresis_below
		|| InCase(After("\p{L}\p{M}*") thenSend(Char(0x325)) elseSend("%"))
...
$$::InCase(Map("a̰ ᶏ", "ɑ̰ ᶐ", "ɗ̰ ᶑ", "ḛ ᶒ", "ɛ̰ ᶓ", "ɜ̰ ᶔ", "ə̰ ᶕ", "ḭ ᶖ", "ɔ̰ ᶗ", "ʃ̰ ᶘ", "ṵ ᶙ", "ʒ̰ ᶚ"))  ; tilde below  --> retroflex hooks (This is a non-looping rota.)
		|| InCase(Map("\ $", "\x{329} \x{32F} \x{330}"))		;|	◌̩ ◌̯ ◌̰		 combining_vertical_line_below |  combining_inverted_breve_below |  combining_tilde_below
		|| InCase(After("\p{L}\p{M}*") thenSend(Char(0x329)) elseSend("$"))

; Key: Colon ( : )
$+;::InCase(LoopMap("ːː ː ˑ"))
	|| InCase(Map("\ :") elseSend("ː"))	; modifier_letter_triangular_colon (x2) |  modifier_letter_triangular_colon |  modifier_letter_half_triangular_colon
