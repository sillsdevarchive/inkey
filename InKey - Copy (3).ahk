...
Alt_Bin=C:\Program Files (x86)\AutoHotkey\Compiler\AutoHotkeySC.bin
No_UPX=1
Execution_Level=2
[VERSION]
Set_Version_Info=1
...
ProcessTinkerFile(kk) {  ; Generate an AHK file from the TINKER file
	...
	; Pull out and process any key handlers for CapsKeys
	if (CapsSensitive) {
		outputdebug CapsKeys = %CapsKeys%
		loop parse, CapsKeys
		{
			local key := A_LoopField   ; TODO: Also handle key in 【 】
			local mLo := RegExMatch(tinkerLines, rulesRE(key), matchLo)
			local mUp := RegExMatch(tinkerLines, rulesRE("+" key), matchUp)
			if (mLo and mUp) {
				if (mLo < mUp)
					tinkerLines := SubStr(tinkerlines, 1, matchLo.Pos(0)-1) . SubStr(tinkerLines, matchLo.Pos(0)+matchLo.Len(0), matchUp.Pos(0)-matchLo.Pos(0)-matchLo.Len(0)) . SubStr(tinkerlines, matchUp.Pos(0)+matchUp.Len(0))
				else
					tinkerLines := SubStr(tinkerlines, 1, matchUp.Pos(0)-1) . SubStr(tinkerLines, matchUp.Pos(0)+matchUp.Len(0), matchLo.Pos(0)-matchUp.Pos(0)-matchUp.Len(0)) . SubStr(tinkerlines, matchLo.Pos(0)+matchLo.Len(0))
				loTranslate := translateRules(matchLo.Value(2))
				upTranslate := translateRules(matchUp.Value(2))
				FileAppend % "$" key "::`n$+" key "::`nif (UseUpperCase()) {`n`t" upTranslate "} else {`n`t" loTranslate "}`nreturn`n", %AHKFile%
			} else if (mLo or mUp) {
				tinkWarnings += 1
				warnTxt := "CapsSensitive = Yes, and CapsKeys included '" key "', but this requires handlers for both " key " and +" key "`n"
				outputdebug Warning: %warnTxt%. mLo=%mLo%  mUp=%mUp%
	...
	while (StrLen(tinkerLines) > 1) {
		...
		if (RegExMatch(tinkerLines, "O)^(?:(\S+)|(?:【([^】]+)】))\s*>\s*(\S.*(\r?\n\s+.*)*\r?\n?)", match)) {
			tinkerLines := SubStr(tinkerLines, match.Len(0) + 1)
			local keystroke := match.Value(1) . match.Value(2)
			StringReplace keystroke, keystroke, %A_Space%, `r`n, -1
			if (keystroke = "00")
				continue	; User blanked out hotkey, so ignore this and all its rules.
			while (RegExMatch(keystroke, "Om)^([!#<>^]*)(A-Z)$", match2)) {

			}
			keystroke := RegExReplace(keystroke, "m)^.*$", "$$$0::")
			FileAppend %keystroke%, %AHKFile%
			local ruleStr := translateRules(match.Value(3))
			FileAppend %ruleStr%, %AHKFile%
			continue
		}
